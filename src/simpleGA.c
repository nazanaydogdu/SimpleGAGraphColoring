...
#define EPSILON  1e-6
#define PI 3.1415927
#define MAXVECSIZE 500
#define MAXPOPSIZE 3000
#define TRUE 1
#define FALSE 0
#define BLX 0
#define SBX 1
  ...
  if ((REPORT) || (num==max_gen))
    {
      ...
      fprintf(fp,"\nBest ever = %d -> fitness: %f (from generation : %d)",
	      (int)best_ever.x[0],best_ever.obj,best_ever_gen);
      for (j=1; j<=num_var-1; j++)
	fprintf(fp,"\n            %d",(int)best_ever.x[j]);
      if (num_discr_var) {
    	  fprintf(fp,"\nBest_ever String = ");
...
float *x;
{
  ...
  int i, j;
   ...
   /* Compare each variable's value with the rest of chromosome
    * Start j from i+1 in order to prevent term3 to increase more than one time
    * for symetric indices like edges[6,2] and edges[2,6]*/
	 for(i = 0; i < MAXVECSIZE; i++){
		 for(j = i + 1; j < MAXVECSIZE; j++){
			 if((int)x[i] == (int)x[j]){
	    		 if(edges[i+1][j+1] == 1){
	    			 term3++;
	    		 }
...
readFile(){
	...
	FILE *ofp;
		...
		int j;
		    ...
		    ifp = fopen("probleminput.txt","r+");
		    ofp = fopen("inputarray.txt","w+");

		    if (ifp == NULL) {
		      printf("Can't open input file probleminput.txt!\n");
		      exit(1);
		    }

		    fscanf(ifp, "%d", &nodeCount);
		    printf("Number of vertices :%d\n", nodeCount);
		    while ((fscanf(ifp, "%d %d", &node1, &node2) != EOF)) {
		    	edges[node1][node2] = 1;
		    	edges[node2][node1] = 1;
		    	node1 = 0;
		    	node2 = 0;
